...
public class GameWebSocket {
    ...
    public Game createGame(String title,String duration,int maxPlayers){
        ...
        System.out.println(CardUtility.getAllSets(table, true));
    ...
    public String verifyChosenSet(int gameId,
        ...
        boolean isSet = CardUtility.setExists(set);
        if(isSet == false)
            return "Not_Set";
        ...
        selectedGame.setTable(CardUtility.removeCards(position, selectedGame.getTable()));
        selectedGame.setRound(selectedGame.getRound()+1);
        
        
        //if there are cards in deck, return response 'ok' and id of 3 cards
        List<Object> list = CardUtility.issue3Cards(position,selectedGame.getDeck(), selectedGame.getTable());
        selectedGame.setDeck((List<Card>) list.get(0));
        selectedGame.setTable((Card[]) list.get(1));
        
        if(!CardUtility.setExists(selectedGame.getTable())){
            return "EndGame";
        }
